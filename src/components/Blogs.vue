    ...
    <div class="space-y-2">
      <div class="space-y-2" v-for="blog in blogs" :key="blog.name">
        <a :href="blog.url" target="_blank" class="flex">
          <img v-if="blog.url.includes('dev.to')" src="/@/assets/icons/dev-dot-to.svg" class="h-6 w-6 mr-2" />
          <img v-else-if="blog.url.includes('medium.com')" src="/@/assets/icons/medium.svg" class="h-6 w-6 mr-2" />
        ...
        <div v-for="post in blog.latestPosts" :key="post.link">
          <a :href="post.link" target="_blank" class="flex flex-row grow">
            ...
            <div>
              <p>{{ post.title }}</p>
              <p class="text-sm text-gray-700">{{ post.publishedAt }}</p>
            </div>
          </a>
        </div>
      </div>
